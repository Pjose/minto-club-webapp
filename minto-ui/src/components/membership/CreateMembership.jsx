...
const CreateMembership = (props) => {
    ...
    const addRelativeToArray = (newRelative) => {
        ...
        setFormData(prev => ({
            ...prev,
            relatives: [...prev.relatives, relative]
        }))
    }
    ...
    const removePersonFromArray = (arrayName, index) => {
        if(arrayName === 'referees') {
            removeRefereeFromArray(arrayName, index)
        } else {
            setFormData(prev => ({
                ...prev,
    ...
    const removeRefereeFromArray = (arrayName, index) => {
        setFormData(prev => ({
            ...prev,
            [arrayName]: prev[arrayName].filter((_, i) => i !== index)
        }))
        setSelectedReferees(prev => prev.filter((_, i) => i !== index))
    }
    ...
    const cancel = async () => {
        const confirmation = await showConfirmation("Are you sure you want to cancel this 'Create Member Application'?")
        if(confirmation) {
            setFormData(DEFAULT_APPLICATION)
            console.log("Create Member Application Cancelled! The form is reset.")
            toast.info("'Create Member Application' -> Cancelled!", {
                description: 'The form is reset.',
            })
            navigate('/login')
        } else {
            console.log("Cancel Aborted! Continue working on the Create Member Application.")
            toast.info("Cancel -> Aborted!", {
                description: "Continue working on the 'Create Member Application'.",
            })
        }
    }
    ...
    <>
        {
            isAuthenticated && isAdminOrStaff ? (
                <>
                    ...
                    <div className="card-footer">
                        <div className="text-center my-2">
                            ...
                            <button type='button' onClick={cancel} className="btn btn-outline-danger mx-3" disabled={loading} title='Cancel Create Member Application'>
                            ...
                            {currentStep < (steps.length) ? (
                                ...
                                <button type="submit" onClick={(e) => onSubmit(e)} className="btn btn-success mx-3" disabled={loading} title='Submit Create Member Application'>
                ...
                <div className="container my-3 p-2">
                    <h3 className="text-primary text-center">Checking credentials...</h3>
                    { !isAdminOrStaff && (<h4 className="text-primary text-center">Unauthorized user</h4>)}
                </div>
            )
        }
